...
ASRCARM = Startup.S swi_handler.S
...
EXTRAINCDIRS = ./uip/ ./uip/apps/ ./uip/uip_app_registration/ ./qr/ ./trezor-crypto/ ./nxp/ ./chan-fs/
...
LDFLAGS +=-T$(LINKERSCRIPTPATH)/$(LINKERSCRIPT)
...
HEXSIZE = $(SIZE) -x --target=$(FORMAT) $(TARGET).hex
